...
%        with the MATLAB sparse solver (default)                          %
...
dt=sigma*dt_ns;                         % [IMPL] time step (stability) [s]
...
% [IMPL] Total number of equations for a single species
ne=(nx+2)*(ny+2);
...
% [IMPL] Define sparsity pattern
pattern=sparse(ne,ne);
for k=1:ne
    pattern(k,k)=1;
end
for i=2:nx+1
...
for is=1:nsteps
    ...
    % [IMPL] Setting the matrix M (common to every species)
    M = pattern;
    for i=2:nx+1
            for j=2:ny+1
    ...
    % [IMPL] Setting the matrices for single species
    MA = M; MB = M; MC = M;
    for i=2:nx+1
            for j=2:ny+1
    ...
    % [IMPL] Setting the RHS vectors
    bA=CAo(:);
    bB=CBo(:);
    bC=CCo(:);
    
    % [IMPL] Setting the RHS vectors for single species
    for i=2:nx+1
            for j=2:ny+1
                k = (nx+2)*(j-1)+i;
    ...
    % [IMPL] Solve the linear systems
    CA = MA\bA;
    CB = MB\bB;
    CC = MC\bC;
    
    % [IMPL] Reshape the solution
