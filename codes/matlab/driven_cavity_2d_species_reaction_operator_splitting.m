...
CB=zeros(nx+2,ny+2);
CC=zeros(nx+2,ny+2);
CAstar=zeros(nx+2,ny+2);    % temporary (after transport)
CBstar=zeros(nx+2,ny+2);    % temporary (after transport)
CCstar=zeros(nx+2,ny+2);    % temporary (after transport)
            ...
            for j=2:ny+1
                ...
                elseif (method == 3)
                        ...
                        if (CAstar(i,j)>1e-12)
                            ...
                            CA(i,j) = (CAstar(i,j)-CBstar(i,j))/...
                                      (1-CBstar(i,j)/CAstar(i,j)*...
                                      exp(-kappa*dt*(CAstar(i,j)-CBstar(i,j))));
                            CB(i,j) = CBstar(i,j)-(CAstar(i,j)-CA(i,j));
                            CC(i,j) = CCstar(i,j)+(CAstar(i,j)-CA(i,j)) + ...
                                                  (CBstar(i,j)-CB(i,j));
